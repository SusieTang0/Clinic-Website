...
function displayAppointments(theStatus, appointmentList) {
  ...
  for (let i = 0; i < appointmentList.length; i++) {
    if (appointmentList[i].status == theStatus || theStatus == 'total' || theStatus == 'Feedback') {
      ...
      let theday = new Date();    
      let apptDate = new Date(appointmentList[i].apptDate);
      if (appointmentList[i].status === 'uncompleted') {
        ...
        } else if(apptDate === theday){
          appointments += `<td><button class="btn-feedback" disabled style="background-color: lightgrey;">Visit Today</button></td></tr>`;
        }else{
          appointments += `<td><button class="btn-feedback" disabled style="background-color: LightCoral;">Missed & Lapsed</button></td></tr>`;
        }
